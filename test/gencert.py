...
def create_cert(subject_names, ca, issuer_key, issuer_cert, usage, validity):
    ...
    if usage is not None:
        ...
        if Usage.CLIENT in usage:
            auths.append(x509.oid.ExtendedKeyUsageOID.CLIENT_AUTH)
        if Usage.SERVER in usage:
            auths.append(x509.oid.ExtendedKeyUsageOID.SERVER_AUTH)

        builder = builder.add_extension(x509.ExtendedKeyUsage(auths),
                                        critical=True)
...
def create_crl(issuer_cert, issuer_key, last_update, next_update,
    ...
    for revoked_cert in revoked_certs:
        ...
        revoked_cert_entry = \
            revoked_cert_entry_builder.build(default_backend())
