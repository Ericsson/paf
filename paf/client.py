...
class TransactionError(Error):
    def __init__(self, reason=None):
        if reason != None:
            ...
            message = "Protocol transaction failed for unknown reason." % reason
...
class Client:
    ...
    def sync_request(self, ta_type, request_args, request_optargs,
                      call_cls):
        call = call_cls(self)
        ta_id = self.async_request(ta_type, request_args, request_optargs, call)
        assert ta_id != None
        call.ta_id = ta_id
        return call.get()
    def async_request(self, ta_type, request_args, request_optargs,
        ...
        request_msg = transaction.produce_request(request_args, request_optargs,
                                                  response_cb)
    ...
    def try_receive(self):
        try:
            ...
            if not ta_id in self.transactions:
